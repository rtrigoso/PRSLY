...
test:
	...
	bash tests/test_adjectives.sh

build: clean $(OUT)/prsly

$(OUT)/prsly: app/pos_classifier.awk app/run_pos.awk
	mkdir -p $(OUT)
	printf '#!/bin/sh\nset -e\n\n' > $@
	printf 'VERSION="%s"\n\n' $(VERSION) >> $@
	printf 'case "$$1" in --version|-v) echo "prsly $$VERSION"; exit 0;; esac\n\n' >> $@
